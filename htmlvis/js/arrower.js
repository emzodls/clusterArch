...
Arrower.drawClusterToolsSVG = (function(cluster, maxCluster,height = 40) {
  var container = document.createElement("div");
  var draw = SVG(container).size('100%', height).group();
  var scale = (function(val) { return parseInt($(window).width() * (val / maxCluster) * 0.85); })
  ...
  if (cluster.hasOwnProperty("orfs")) {
      ...
      if (orf.hasOwnProperty("domains")) {
        ...
        for (var j in orf.domains) {
          ...
          $(dom.node).mouseover({domain: domain}, function(handler){
            ...
            Arrower.showToolTip("Domain: " + handler.data.domain.code + " (" + handler.data.domain.bitscore + ")" + "<br/>" + start + " - " + end, handler);
            $(handler.target).css("stroke-width", "3px");
            $(handler.target).css("stroke", "red");
            handler.stopPropagation();
          });
    ...
    var svgObj = $(container).find("svg")[0];
  return svgObj;
});
...
Arrower.getArrowPointsDom = (function(orf, cluster, height, scale) {
  var points = [];
  var pts = Arrower.getOrfPoints(orf, cluster, height, scale);
...
Arrower.getDomainPoints = (function(domain, orf, cluster, height, scale) {
  var points = [];
  var arrow_pts = Arrower.getArrowPointsDom(orf, cluster, height, scale);
  if (orf.strand < 0) {
